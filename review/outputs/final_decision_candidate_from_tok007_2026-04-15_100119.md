...
| 会议纪要路径 | `review/outputs/`（历史附件归档；现行机审稿引用以上方约束为准） |
...
| M-018 | stage_quality_gate_pass_pct | =100% | 演练通过 | 待连续7天观测 | 有条件通过 | `reports/archive/gate_verification/stage_gate_drift_drill_report_2026-03-27.md` | 孙悦（QA） |
| M-019 | requirement_traceability_coverage_pct | =100% | 进行中 | 待连续7天观测 | 不通过 | `reports/supply_traceability_matrix_2026-03-25.csv` | 孙悦（QA） |
| M-021 | token_runtime_readiness_pct | =100% | 100%（开发阶段） | 待实测 | 有条件通过 | `reports/archive/gate_verification/token_runtime_readiness_*.md` + `reports/token_runtime_implementation_gap_review_2026-03-30.md` | 王磊（ARCH）+周敏（SEC） |
...
| F-03 | P1 | 补齐 M-017/M-018/M-019 连续7天趋势证据 | 李娜（PLAT）+PMO | 2026-04-05 | `reports/dependency/` 与 `reports/archive/gate_verification/` 趋势报告 |
...
## 附录：TOK-007 自动复审回填（2026-04-15_100119）

1. 自动复审来源：`/home/long/project/立交桥/review/outputs/tok007_release_recheck_2026-04-15_095012.md`
2. 自动复审结论：`CONDITIONAL_GO`
3. 说明：该候选稿用于人工审阅与签署准备，不直接替代正式签署版本。
4. 现行机审稿指针：`review/outputs/current_machine_review_sources.md`
