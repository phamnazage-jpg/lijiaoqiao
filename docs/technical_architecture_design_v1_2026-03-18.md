...
>
> ⚠️ **废弃说明**：本文档中包含 Kafka、Istio 等**早期设计讨论**，实际代码实现时已弃用。代码库中无 Kafka/etcd/Redis 运行时依赖。请以 `technical_architecture_optimized_v2_2026-03-18.md` 和实际代码为准。
...
| 缓存 | Redis | 7.x | 缓存+限流 |
| 消息队列 | ~~Kafka 3.x~~ | **未使用**（单体架构，PostgreSQL Outbox 替代） |
